...
export function createAss(
  ...
  const { arabicY, translationY } = captionPositions(
    arabicFontSize,
    translationFontSize,
    captionGap,
  );
...
`;
  ...
  for (const word of words) {
    ...
    events.push(
      `Dialogue: 0,${start},${end},Quran,,0,0,0,,{\\an5\\pos(${CAPTION_CENTER_X},${arabicY})}${contextLine(word, index, wordsPerCaption)}`,
    );
    events.push(
      `Dialogue: 1,${start},${end},Translation,,0,0,0,,{\\an5\\pos(${CAPTION_CENTER_X},${translationY})}${escapeAss(word.wordTranslation)}  •  ${word.verseKey}`,
    );
  }
