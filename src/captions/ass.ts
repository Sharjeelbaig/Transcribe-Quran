...
`;
  ...
  for (const word of words) {
    ...
    for (const segment of animationSegments(arabicVisual, captionStart, captionEnd)) {
      events.push(
        `Dialogue: 0,${assTime(segment.start)},${assTime(segment.end)},Quran,,0,0,0,,{${transitionTag(arabicPosition, arabicFontSize, arabicVisual, segment.phase, segment.animation, segment.end - segment.start)}}${contextLine(word, index, wordsPerCaption, options.arabicColor, options.arabicColor?.toUpperCase() === "#FFFFFF" ? "#FFD700" : options.arabicColor)}`,
      );
    }
